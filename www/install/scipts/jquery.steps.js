...
;(function ($, undefined)
{
    ...
    function initialize(options)
    {
        ...
        return this.each(function ()
        {
            ...
            //wizard.triggerHandler('stepChanging', [currentIndex = 0]);
    ...
    function paginationClickHandler(event)
    {
        ...
        switch (href.substring(href.lastIndexOf("#")))
        {
            ...
            case "#submitForm":
                wizard.triggerHandler('submitForm', [state.currentIndex]);
                break;
        }
    }
    ...
    function refreshPagination(wizard, options, state)
    {
        if (options.enablePagination)
        {
            var finish = wizard.find(".actions a[href$='#finish']").parent(),
                ...
                submitForm = (options.enableSubmitButton) ? wizard.find(".actions a[href$='submitForm']").parent() : false,
                showSubmit = (options.enableSubmitButton && options.stepsForSubmit.indexOf((state.currentIndex+1).toString()) != -1);
            ...
            else if(showSubmit)
            {
                next._hideAria();
                finish._hideAria();
                submitForm._showAria()._enableAria().children().removeClass('disabled');
            }
    ...
    function registerEvents(wizard, options)
    {
        ...
        wizard.bind("submitForm" + eventNamespace, options.onSubmitForm);
    ...
    function renderPagination(wizard, options, state)
    {
        if (options.enablePagination)
        {
            var pagination = "<{0} class=\"actions {1}\"><ul role=\"menu\" aria-label=\"{2}\">{3}</ul></{0}>",
                buttonTemplate = "<li><a id=\"btn{0}\" href=\"#{0}\" role=\"menuitem\">{1}</a></li>",
                buttons = "";
            ...
            {
                buttons += format(buttonTemplate, "submitForm", options.labels.submitForm  )
            }
    ...
    function stepClickHandler(event)
    {
        ...
        if ($(this).parent().hasClass('disabled') != true && $(this).parent().hasClass('current') != true)
        {
    ...
    $.fn.steps.disableButton = function(button)
    {
        switch (button)
        {
            case 'next':
                return $("#btnnext").addClass('disabled').parent()._disableAria();
            case 'previous':
                return $("#btnprevious").addClass('disabled').parent()._disableAria();
            case 'submit':
                return $("#btnsubmitForm").addClass('disabled').parent()._disableAria();
            case 'finish':
                return $("#btnfinish").addClass('disabled').parent()._disableAria();
        }

        return false;
    };
    
    $.fn.steps.enableButton = function(button)
    {
        switch (button)
        {
            case 'next':
                return $("#btnnext").removeClass('disabled').parent()._enableAria();
            case 'previous':
                return $("#btnprevious").removeClass('disabled').parent()._enableAria();
            case 'submit':
                return $("#btnsubmitForm").removeClass('disabled').parent()._enableAria();
            case 'finish':
                return $("#btnfinish").removeClass('disabled').parent()._enableAria();
        }
        return false;    
    };
    
    $.fn.steps.hideButton = function(button)
    {
        switch (button)
        {
            case 'next':
                return $("#btnnext").parent()._disableAria()._hideAria();
            case 'previous':
                return $("#btnprevious").parent()._disableAria()._hideAria();
            case 'submit':
                return $("#btnsubmitForm").parent()._disableAria()._hideAria();
            case 'finish':
                return $("#btnfinish").parent()._disableAria()._hideAria();
        }
        return false;
    };
    
    $.fn.steps.showButton = function(button)
    {
        switch (button)
        {
            case 'next':
                return $("#btnnext").parent()._enableAria()._showAria();
            case 'previous':
                return $("#btnprevious").parent()._enableAria()._showAria();
            case 'submit':
                return $("#btnsubmitForm").parent()._enableAria()._showAria();
            case 'finish':
                return $("#btnfinish").parent()._enableAria()._showAria();
        }
        return false;
    };

    $.fn.steps.setStepToError = function()
    {
        var wizard = $(this),
            state = getState(wizard);
        wizard.find(".steps li").eq(state.currentIndex).addClass("error");
    };
    $.fn.steps.removeStepError = function()
    {
        var wizard = $(this),
            state = getState(wizard);
        wizard.find(".steps li").eq(state.currentIndex).removeClass("error");
    };
    $.fn.steps.validateCurrentStep = function()
    {
        var wizard = $(this),
            state = getState(wizard);
        wizard.triggerHandler('stepChanging', (state.currentIndex));
    };
    ...
    var defaults = $.fn.steps.defaults = {
        ...
        onSubmitForm: function (event, currentIndex) { },
