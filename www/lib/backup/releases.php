...
class Releases
{
	...
	public function processReleasesStage1($groupID, $echooutput=true)
	{
        ...
        // if ($this->echooutput)
            echo "Searching collections...This could take a little while.\n";

        /* The even newer, and more improved way of doing things:
         * First, we're going to do a check against all binaries. If they have all of the required parts,
         * then we'll go ahead and get a file size by summing the sizes of the parts.  Set partcheck to 1 and we're done.
         *
         * Secondly, we'll compare determine if collections have all of the required parts, based on binaries
         * that have been part checked.  This eliminates an unnecessary join query below.
         *
         * */
        ...
        // --------------NEW WAY--------------
        // Grab all collections where filecheck is either 0 or 1 (they would be 1 if scanned previously but not all parts were
        // available for the binaries.  We'll set filecheck to 15 if the first binary is filenumber 1 and we have all the files,
        // or set filecheck to 16 if the first binary is 0 and we have all the files.
        // If the first available binary is not filenumber 0 or 1, or we don't have an adequate number of files, we won't mess with the collection.

        $collectionsresult=$db->queryDirect("SELECT ID, totalFiles FROM collections WHERE totalFiles!=0 AND filecheck IN (0, 1)");
        $collectionstotal=$db->getNumRows($collectionsresult);
        // if($this->echooutput)
            Echo "Found ".$collectionstotal." collections to check.\n";
        ...
        if ($collectionstotal>0)
        {
            ...
            {
                $binaryrows=$db->queryDirect("SELECT ID FROM binaries WHERE collectionID=".$collectionrow["ID"]);
                $binarycount=$db->getNumRows($binaryrows);
                // $binrow=$db->fetchAssoc($binaryrows);

                if($binarycount>=$collectionrow["totalFiles"])
                {
                    // Echo "Setting File Check to 15 (filenumber 1)\n";
                    $db->query("UPDATE collections SET filecheck=15 WHERE ID=".$collectionrow["ID"]);
                    $colsupdated++;
                }
        ...
        // Below is the old, slow way of doing things that required temporary tables and notoriously slow subqueries that MySQL doesn't optimize well
		...
		if (empty($groupID))
		{
            ...
            if($echooutput)
                Echo "Getting list of binaries to check...\n";
            $binaryresult = $db->queryDirect("SELECT b.ID AS bID, b.totalParts FROM binaries AS b INNER JOIN collections AS c ON b.collectionID=c.ID WHERE b.partcheck = 0 AND c.filecheck=15");
            $binaryrowcount = $db->getNumRows($binaryresult);

            if($binaryrowcount>0);
            {
                if($echooutput)
                    Echo "Checking {$binaryrowcount} binaries for parts completion...\n";
                $bincount = 0;
                $binsprocessed = 0;
                $timeQueryParts = 0;
                $timeUpdateBins = 0;
                $db->setAutoCommit(FALSE);
                while ($binaryrow = $db->fetchAssoc($binaryresult))
                {
                    $binsprocessed ++;
                    if($echooutput)
                        $consoletools->overWrite("Binaries processed: ".$consoletools->percentString($binsprocessed, $binaryrowcount)." Working on binaryID ".$binaryrow['bID']);
                    $tempTime1 = microtime(true);
                    $partresult = $db->queryDirect("SELECT ID FROM parts AS p WHERE p.binaryID=".$binaryrow["bID"]);
                    $timeQueryParts = $timeQueryParts + (microtime(true) - $tempTime1);
                    If($db->getNumRows($partresult)>=$binaryrow['totalParts'])
                    {
                        $tempTime2 = microtime(true);
                        $db->queryDirect("UPDATE binaries SET partcheck=1 WHERE ID=".$binaryrow["bID"]);
                        $timeUpdateBins = $timeUpdateBins + (microtime(true) - $tempTime2);
                        $bincount++;
                    }
                ...
                if($echooutput)
                {
                    $avgBinUpdateTime = 0;
                    $avgPartQueryTime = 0;
                    if ($binsprocessed>0) {$avgPartQueryTime = $timeQueryParts/$binsprocessed;}
                    if ($bincount>0) {$avgBinUpdateTime = $timeUpdateBins/$bincount;}
                    if ($avgPartQueryTime && $avgBinUpdateTime)
                    {
                        Echo "\nAverage query time: ".($timeQueryParts/$binsprocessed)."   Average update time: ".($timeUpdateBins/$bincount);
                        Echo "\nCommitting database changes... ".$bincount." binaries were updated.\n";
                    }
                    else echo "\nCommitting database changes... ".$bincount." binaries were updated.\n";
                }
            ...
            unset($binsprocessed, $bincount, $binaryrow, $partresult);
            unset($binaryresult, $binaryrowcount);
		}
		else
		{
        ...
        if($echooutput)
            echo "\nUpdating file check status in collections...\n";
		$db->query("UPDATE collections c SET filecheck = 2 WHERE c.ID IN (SELECT b.collectionID FROM binaries b WHERE c.ID = b.collectionID AND b.partcheck = 1 GROUP BY b.collectionID
						HAVING count(b.ID) >= c.totalFiles) AND c.filecheck=15 ".$where);

		// Set file check to 1 if we don't have all the parts.
		$db->query("UPDATE collections SET filecheck = 1 WHERE filecheck=15 ".$where);
	...
	public function processReleasesStage2($groupID, $echooutput=true)
	{
		...
		if ($this->echooutput)
			echo $n."\033[1;33mStage 2 -> Get the size in bytes of the collections.\033[0m".$n;
		$stage2 = TIME();
        ...
        $collectionsresult=$db->queryDirect("SELECT ID FROM collections AS c WHERE c.filecheck = 2 AND c.filesize = 0");
        $collectioncount=$db->getNumRows($collectionsresult);
        $colsupdated=0;
        if($collectioncount)
        {
            if($echooutput)
                echo "Getting file sizes for ".$collectioncount." collections.\n";
            $colstotal=0;
            $db->setAutoCommit(FALSE);
            while($collectionsrow=$db->fetchAssoc($collectionsresult))
            {
                $collectionsize=$db->queryDirect("SELECT SUM(size) as colsize FROM parts AS p WHERE p.collectionID=".$collectionsrow["ID"]);
                $colsizeresult = $db->fetchAssoc($collectionsize);

                If($colsizeresult["colsize"]>0)
                {
                    $db->query("UPDATE collections SET filesize=".$colsizeresult["colsize"].", filecheck=3 WHERE ID=".$collectionsrow["ID"]);
                    $colsupdated++;
                }
                $colstotal++;
                if($this->echooutput)
        ...
        if($colsupdated==0)
        {
            ...
            $filecheck3 = $db->queryDirect("SELECT ID FROM collections where filecheck IN (3,4) AND filesize>0");
	...
	public function processReleasesStage3($groupID, $echooutput=true)
	{
		...
		else
		{
			if($db->queryDirect("SELECT ID from collections where filecheck = 3 and filesize > 0"))
			{
				...
				if ($maxfilesizeres['value'] != 0)
				{
					$db->query(sprintf("UPDATE collections SET filecheck = 5 WHERE filecheck = 3 AND filesize > %d ", $maxfilesizeres['value']));
	...
	public function processReleases($categorize, $postproc, $groupName, $echooutput=true)
	{
        ...
        // $deletedCount = $this->processReleasesStage7b($groupID, $echooutput=true);
