...
jQuery(function ($) {
    ...
    var login_left = $("#menu-login").css("left");
    $("#menu-login").click(function (e) {
        var login_pos = $("#menu-login").offset();
        if ( $( "#login-box" ).is( ":hidden" ) ) {
            $( "#login-box" ).css("left", login_pos.left-100)
    ...
    $("#login-button").click( function (e) {
       var ok_to_login = true;
       if($("#username").val() == '') {
           $("#username").addClass("loginbox-error");
           ok_to_login = false;
       }
       if($("#password").val() == '') {
           $("#password").addClass("loginbox-error");
           ok_to_login = false;
       }
       if(ok_to_login === true) {
           $(this).parent($(this)).submit();
       }
    ...
    $("form#login-form").submit( function (event) {
        ...
        var posting = $.post( "login_proc", dataString);
        posting.done( function(result){
                var resultArray = jQuery.parseJSON(result);
                if(resultArray['login'] === checkCookie()) {
                    window.location='/index.php';
                } else {
                    // alert("We hit a bad spot.");
                    $("#login-failed").text(resultArray['login']);
    ...
    {
        var loginGuid=getCookie("loginGuid");
        if (loginGuid!=null && loginGuid!="")
        {
            return loginGuid;
        }
        else
    ...
    $("#username, #password").keypress( function (e) {
        if($(this).hasClass("loginbox-error")) {
            $(this).removeClass("loginbox-error")
        }
    });
    ...
    $("table.data a.modal_nfo").colorbox({     // NFO modal
        ...
        innerWidth: "800px", innerHeight: "90%", initialWidth: "800px", initialHeight: "90%", speed: 0, opacity: 0.9
    });
    // Screenshot modal
    ...
    $("table.data a.modal_imdb").colorbox({    // IMDB modal
        ...
        title: function () { return $(this).parent().parent().children('a.title').text(); },
        innerWidth: "800px", innerHeight: "450px", initialWidth: "800px", initialHeight: "450px", speed: 0, opacity: 0.9
    }).click(function () {
