...

{
    function deleteReleases($sql, $type)
    {
        ...
        foreach ($sql as $rel)
        {
            if ($delete == 1)
            {
                if($type == 'Size')
                {
                    $size = "(" . number_format((($rel['size']/1024)/1024), 1) . " MB) Grp: " . $rel['groupID'] . " ";
                }
                else
                    $size = '';
                echo $type . ": " . $size . $rel['searchname']."\n";
    ...
    function deleteExecutable($and)
    {
        $type = "Executable";
        $db = new DB();
        $sql = $db->query('select r.ID, r.guid, r.searchname from releases r left join releasefiles rf on rf.releaseID = r.ID where (rf.name like "%.exe%" OR rf.name like "%.scr%") and r.categoryID not in (4000, 4010, 4020, 4050, 7010)'.$and);
        $delcount = deleteReleases($sql, $type);
        return $delcount;
    }
    ...
    function deletePassworded($and)
    {
        $s = new Sites();
        $site = $s->get();
        if($site->deletepasswordedrelease != '1')
        {
            echo "Deletion of password protected releases disabled in Site settings.\n";
            return 0;
        }
        $type = "Passworded";
        $db = new DB();
    ...
    function deleteSize($and)
    {
        $type = "Size";
        $db = new DB();
        $sql = $db->query("select ID, guid, searchname, size, groupID from releases where totalPart = 1 and nzbstatus=1 and size < 1000000 and categoryID not in (8000, 8010, 8020, 8030, 8040, 8050, 8060, 3010)".$and);
        $delcount = deleteReleases($sql, $type);

        // Check releases against category minimum sizes
        $sql = $db->query("SELECT r.ID, guid, searchname, size, groupID FROM releases AS r LEFT JOIN
