...
cur = con.cursor()
print("\nGetting partrepair value.")
cur.execute("select value from site where setting = 'partrepair'")
torun = cur.fetchone()
if int(torun[0]) != 3:
 	sys.exit("Manual Part Repair is disabled.")
...
print("\nRetrieving the list of groups from partrepair.  This may take a few minutes.\n")
cur.execute("SELECT DISTINCT(partrepair.groupID) FROM partrepair INNER JOIN groups ON partrepair.groupID=groups.ID ORDER BY groups.last_repair ASC")
datas = cur.fetchall()
	...
	def run(self):
		...
		while True:
			try:
				...
				if my_id:
					time_of_last_run = time.time()
					subprocess.call(["php", pathname+"/../nix_scripts/tmux/bin/binaries_manual.php", ""+my_id])
					time.sleep(.5)
...
def main():
	...
	for groupIDs in datas:
		my_queue.put("%s" % (groupIDs[0]))
