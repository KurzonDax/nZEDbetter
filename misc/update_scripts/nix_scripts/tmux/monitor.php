...
printf($mask, "XXX","0(0)",number_format($porn_releases_now)."(".$porn_percent."%)");
...
while( $i > 0 )
{
	...
	if ( @$initquery[0]['xxx'] != NULL ) { $porn_releases_now = $initquery[0]['xxx']; }
    ...
    if ( @$proc_tmux_result[0]['run_purge_thread'] != NULL ) { $run_purge_thread = $proc_tmux_result[0]['run_purge_thread']; }
    if ( @$proc_tmux_result[0]['purge_max_cols'] != NULL ) { $purge_max_cols = $proc_tmux_result[0]['purge_max_cols']; }
	...
	if ( $running == "TRUE" )
	{
		...
		else
		{
            ...
            {
                $color = get_color($colors_start, $colors_end, $colors_exc);
                shell_exec("tmux respawnp -k -t${tmux_session}:0.5 'echo \"\033[38;5;${color}m\n${panes0[5]} has been disabled/terminated by Purge Thread\"'");
            }
